...
Ts = 0.02; % Select the sample time
sysD = ss(A,B,C,D); % Discrete-time state space model

n = size(sysD.A, 1); % For convenience, we save the size of the state dimension
m = size(sysD.B, 2); % and of the input dimension

% We set some bounds for the state and input
LBx = [-pi/2; -4]; % Lower bound for the state
UBx = [pi/2; 4]; % Upper bound for the state
LBu = [-50]; % Lower bound for the input
UBu = [50]; % Upper bound for the input
...
Q = [10 0 ; 0 1];     % Ponderacion de variables de estado
R = 0.1;              % Ponderacion de variables de control
...
x = [-60;0].*pi/180; % Current state of the system
ur = 0; % We select a reference ur
xr = [0;0]; % We compute a state reference xr that is a steady state

tFinal = 100/0.05; % Número total de muestras
xSP = zeros(2,tFinal); % Declaracion variables de estado
uSP = zeros(1,tFinal); % Declaracion variable de control
ySP = zeros(1,tFinal); % Declaracion variable de salida

xSP(:,1) = x;
% We call the solver
for kk = 1:1:tFinal
    [u, k, e_flag, Hist] = spcies_MPCT_EADMM_solver(x, xr, ur, 'sys', sys, 'param', param,'options', options, 'genHist', 2);
    x = sysD.A*x + sysD.B*u;
    uSP(:,kk) = u;
    xSP(:,kk) = x;
end
...
t = 0:1:tFinal-1;
hold on
plot(t,xSP(1,:),'m','LineWidth',1.5);
hold on
line([0,tFinal],[-pi/2,-pi/2],'Color',[0,0,0],'LineStyle','--');
line([0,tFinal],[pi/2,pi/2],'Color',[0,0,0],'LineStyle','--');
line([0,tFinal],[xr(1,1),xr(1,1)],'Color',[0.25,0.25,0.25],'LineStyle',':');
